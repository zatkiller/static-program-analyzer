...
PKBResponse PKB::getRelationship(PKBField field1, PKBField field2, PKBRelationship rs) {
	...
	case PKBRelationship::MODIFIES:
	{
		std::unordered_set<std::vector<PKBField>, PKBFieldVectorHash> extracted = modifiesTable->retrieve(field1, field2);
		return extracted.size() != 0 ? PKBResponse{ true, Response{extracted} } : PKBResponse{ false, Response{extracted} };
		break;
	}
	default:
